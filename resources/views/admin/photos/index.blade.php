                    ...
                    <tr class="hover-actions-trigger {{ $loop->odd ? 'bg-light' : '' }}">
                        ...
                        <td class="align-middle">
                            <img src="{{ asset('images/photos/' . $photo->image) }}"
                                alt="{{ $photo->alt_text }}{{ asset('images/photos/' . $photo->image) }}"
                                alt="{{ $photo->alt_text }}" width="80" height="60"
                        ...
                        <td class="align-middle text-nowrap">
                            {{ \Carbon\Carbon::parse($photo->created_at)->format('d-m-Y') }}</td>

                        <td class="align-middle text-center">
                            <div class="d-flex justify-content-center gap-1">
                                {{-- View --}}
                                <a href="{{ url('admin/photos/' . $photo->id) }}" class="btn btn-sm "
                                    data-bs-toggle="tooltip" title="View">
                                    <i class="fas fa-eye"></i>
                                </a>

                                {{-- Edit --}}
                                <a href="{{ url('admin/photos/' . $photo->id . '/edit') }}" class="btn btn-sm "
                                    data-bs-toggle="tooltip" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>

                                {{-- Delete --}}
                                <form action="{{ url('admin/photos/' . $photo->id) }}" method="POST"
